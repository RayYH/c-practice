/**
 * @see https://www.ibm.com/docs/en/i/7.4?topic=designs-using-poll-instead-select
 */
#include <stdio.h>
#include <stdlib.h>
#include <sys/ioctl.h>
...
int main(void) {
  ...
  fds[0].fd = listen_sd;
  fds[0].events = POLLIN;
  /* Initialize the timeout to 3 minutes. If no activity after 3 minutes this
   * program will end. timeout value is based on milliseconds. */
  timeout = (3 * 60 * 1000);
  ...
  do {
    ...
    for (i = 0; i < current_size; i++) {
      ...
      /* If revents is not POLLIN, it's an unexpected result, log and end the
       * server. */
     ...
     * to move back the events and revents fields because the events will
     * always be POLLIN in this case, and revents is output. */
    if (compress_array) {
